...
func TypeURL(v interface{}) (string, error) {
	...
	if !ok {
		...
		return string(proto.MessageName(pb)), nil
	}
	return u, nil
}

// Is returns true if the type of the Any is the same as v.
func Is(any *anypb.Any, v interface{}) bool {
...
func MarshalAny(v interface{}) (*anypb.Any, error) {
...
func UnmarshalTo(any *anypb.Any, out interface{}) error {
...
func getTypeByUrl(url string) (urlType, error) {
	...
	pt, err := protoregistry.GlobalTypes.FindMessageByURL(url)
	if err != nil {
		return urlType{}, fmt.Errorf("type with url %s: %s: %w", url, err, ErrNotFound)
	}
	return urlType{
		// get the underlying Elem because proto returns a pointer to the type
		t:       reflect.TypeOf(pt.Zero()),
		isProto: true,
	}, nil
}
